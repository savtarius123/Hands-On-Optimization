...
def cheese_empire(
    ...
    beta = pl.LpVariable.dicts("beta", factory, lowBound=0, upBound=1)

    # problem definition and objective function
    prob = pl.LpProblem("cheese_empire_3", pl.LpMinimize)
    prob += (
        ...
        * pl.lpSum(build[l][k] * distance[l][k] for l in location for k in location)
        - 24 * pl.lpSum(capacity[f] * (1 - beta[f]) for f in factory)
    )

    # constraints
    for f in factory:
        prob += pl.lpSum(ship[f][l] for l in location) == produce[f]
        prob += produce[f] <= capacity[f] * beta[f]
    ...
    for f in factory:
        alpha = 1.0 / pl.value(beta[f]) - 1
        print(f"Factory {f} alpha = {alpha}")
